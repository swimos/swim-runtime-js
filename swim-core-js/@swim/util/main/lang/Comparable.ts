...
export const Comparable = {} as {
   ...
   * before `undefined`.
   */
  compare<T>(x: unknown, y: unknown): number;

  /**
   * Returns `true` if `object` conforms to the [[Comparable]] interface.
   */
  is<T>(object: unknown): object is Comparable;
};

Comparable.compare = function (x: unknown, y: unknown): number {
  if (x === void 0) {
    if (y === void 0) {
      return 0;
    } else {
  ...
  } else if (typeof (x as Comparable).compareTo === "function") {
    if (y === void 0 || y === null) {
      return -1;
    } else {
      return (x as Comparable).compareTo(y);
    }
  } else {
    return NaN;
  }
