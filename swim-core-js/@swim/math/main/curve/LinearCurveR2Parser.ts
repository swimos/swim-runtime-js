...
/** @hidden */
export class LinearCurveR2Parser extends Parser<LinearCurveR2> {
  ...
  feed(input: Input): Parser<LinearCurveR2> {
    return LinearCurveR2Parser.parse(input, this.x0Parser, this.y0Parser,
  ...
  static parse(input: Input, x0Parser?: Parser<number>, y0Parser?: Parser<number>,
               x1Parser?: Parser<number>, y1Parser?: Parser<number>,
               command?: number, step: number = 1): Parser<LinearCurveR2> {
    let c = 0;
    if (step === 1) {
    ...
    if (step === 4) {
      ...
      if (y1Parser !== void 0) {
        if (y1Parser.isDone()) {
          ...
          return Parser.done(new LinearCurveR2(x0, y0, x1, y1));
        } else if (y1Parser.isError()) {
          return y1Parser.asError();
        }
      }
    }
    return new LinearCurveR2Parser(x0Parser, y0Parser, x1Parser, y1Parser, command, step);
  }

  static parseRest(input: Input, command?: number, x0Parser?: Parser<number>,
                   y0Parser?: Parser<number>): Parser<LinearCurveR2> {
...
CurveR2Parser.Linear = LinearCurveR2Parser;
